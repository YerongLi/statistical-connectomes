...
aws_access_key_id='AKIAJPL4P4UUIGBQSSMA', aws_secret_access_key='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')
    ...
    prefixes = ["HCP_1200/%s/T1w/%s/"%(subject,x) for x in SERIES_MAP.values()]
    prefixes = tuple(prefixes)
    s3_keylist = [x for x in s3_keylist if x.startswith(prefixes) and (not ('eddylogs' in x or 'grad_dev.nii.gz' in x or 'nodif_brain_mask.nii.gz' in x))]
