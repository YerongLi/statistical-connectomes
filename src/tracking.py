...
try:
  ...
  print('Error: Please input subject directory')
  sys.exit(1);
subject = DATA_dir.split('/')[-1] if not ''==DATA_dir.split('/')[-1] else DATA_dir.split('/')[-2]
diffusion_directory = pjoin(DATA_dir, 'T1w', 'Diffusion')
fiber_file = pjoin(diffusion_directory, subject+'LocalTracking.npz')
#print(fiber_file)
print(subject)
...
img, gtab = read_data(diffusion_directory)
...
#         fiber_file)
sl = np.array([[s[0],s[-1]] if s[0,2]<s[-1,2] else [s[-1],s[0]] for s in streamlines])
np.savez_compressed(fiber_file,endpoints =sl)
print(subject+ ' Tracking Done.')
